...
# * copy to clipboard with any OS : pbcopy < $1 under OSX? see:
...
perl -pi.50 -e  's/^((  )+)(\*\*)/$1\* \*\*/g' $1         # Lines beginning by spaces followed by a **bold text**
perl -pi.60 -e  's/~~(.*)~~/\<del\>$1\<\/del\>/g' $1      # ~~strike through~~
